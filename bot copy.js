...
// 資料儲存
let warningsData = {};
let marriageData = {}; // 結婚資料
let proposalData = {}; // 求婚資料
let mutedMembers = {}; // 追蹤被禁言的成員
...
function saveProposals() {
  try {
    const dataToSave = JSON.stringify(proposalData, null, 2);
    fs.writeFileSync("./proposals.json", dataToSave);
    console.log("求婚資料已成功儲存, 資料數量:", Object.keys(proposalData).length);
    if (Object.keys(proposalData).length > 0) {
      console.log("目前的求婚ID:", Object.keys(proposalData));
    }
  } catch (error) {
...
function saveMutedMembers() {
  try {
    fs.writeFileSync("./muted_members.json", JSON.stringify(mutedMembers, null, 2));
  } catch (error) {
...
async function handleButtonInteraction(interaction) {
  ...
  if (customId.startsWith('accept_')) {
    action = 'accept';
    proposalId = customId.substring(7); // 移除 'accept_' 前綴
  } else if (customId.startsWith('reject_')) {
    action = 'reject';
    proposalId = customId.substring(7); // 移除 'reject_' 前綴
  }
  ...
  if (interaction.user.id !== proposal.target) {
    console.log(`權限檢查失敗，用戶ID: ${interaction.user.id}, 目標ID: ${proposal.target}`);
    await interaction.reply({
      content: "❌ 這不是對你的求婚！",
      flags: 64
    });
...
async function handleProposeCommand(interaction) {
  ...
  console.log('建立新求婚:', { proposalId, proposer: proposer.id, target: target.id });
  saveProposals();
...
async function handleDeleteWarnCommand(interaction) {
  ...
  try {
    const dmEmbed = new EmbedBuilder()
      .setColor("#32CD32")
      .setTitle("✅ 警告已被撤銷")
      .setDescription(`你在 **${interaction.guild.name}** 伺服器中的一個警告已被撤銷`)
      .addFields(
    ...
    try {
      const dmEmbed = new EmbedBuilder()
        ...
        .addFields(
          ...
          { name: "執行管理員", value: interaction.member.displayName },
          { name: "解除時間", value: new Date(unmuteTime).toLocaleString("zh-TW") }
        )
  ...
  try {
    ...
    try {
      const dmEmbed = new EmbedBuilder()
        .setColor("#32CD32")
        .setTitle("🔊 禁言已被解除")
        .setDescription(`你在 **${interaction.guild.name}** 伺服器的禁言已被管理員解除`)
        .addFields(
          { name: "執行管理員", value: interaction.member.displayName },
          { name: "解除時間", value: new Date().toLocaleString("zh-TW") }
        )
...
// 啟動機器人
client.login(config.token);Name("warn")
    .setDescription("警告成員")
    .addUserOption((option) =>
      option.setName("user").setDescription("要警告的成員").setRequired(true)
    )
